...
> 마지막 업데이트: 2026-05-04 (랜딩 페이지, 집중 모드, 글자 크기, 전체 내보내기, 작품 설명 수정, 통계 바, NovelCard 리디자인, 글쓰기 통계 페이지, 일별 작성량 자동 기록, AI 글쓰기 보조, 작품 검색·정렬, 회차 복사·검색, 목표 달성 피드백, 모바일 반응형, 단축키 도움말, 회차 미리보기, 작품 커버 컬러, 저장 상태 개선, 회차 순서 정리, 통계 작품별 현황, 에디터 온보딩, 작품 cascade 삭제, 다크모드 시스템 감지, 에디터 스켈레톤, 내보내기 개선, 회차 일괄 삭제, Firestore 보안 규칙, Error Boundary, PWA, 에디터 회차 제목 인라인 수정, 홈 빈 상태 개선, 통계 차트 개선, 회차 목록 빈 상태 개선, 저장 시간 표시, 글 미리보기 모드, 연간 잔디 뷰, 작품 태그/장르, 검색 확장, 모바일 ⋯ 오버플로우 메뉴, Markdown 내보내기, 공유 링크, 에피소드 목표 진행바, 홈 연속 일수 배지, 노트 고정/검색, 카드 설명 인라인 편집, 집중 타이머, 일일 목표 추적, 공유 링크 만료, PDF 내보내기, Cmd+S/P/Shift+F 단축키, 모바일 회차 순서 변경)
...
| 보안 규칙 | Firestore Security Rules (`firestore.rules`) — 인증된 사용자만 자신의 데이터 접근 |
...
users/{uid}/
  novels/{novelId}/
    title, description, episodeCount, color?, createdAt, updatedAt
    episodes/{episodeId}/
      title, content, order, charCount, excerpt?(첫 80자), createdAt, updatedAt
...
- 검색 인풋 — 제목·설명 포함 실시간 필터링 (작품이 1개 이상일 때 표시)
- 정렬 버튼 — 최근 수정 / 제목 가나다순 / 회차 많은 순 (검색창 우측)
- 작품 생성 폼 (제목 + 설명)
- 작품 목록 (3열 그리드)
...
- 회차 검색 인풋 — 5회차 이상일 때 표시, 제목 실시간 필터링
- 회차 이름 인라인 수정 (✎ 버튼 → Enter 확인, Esc 취소)
- 회차 복사 (⎘ 버튼, 제목에 "(복사본)" 추가, 내용 그대로 복제, 목록 맨 아래에 추가)
...
- `🗒️`: 메모 패널 열기/닫기
- `↓ txt`: 현재 내용을 `.txt` 파일로 다운로드 (sm 이상만)
- `?`: 단축키 도움말 모달 열기
- 다크모드 토글, 로그인 버튼 (sm 이상만)
...
- 버튼 — 오른쪽 (테두리 있는 버튼 스타일, 활성시 인디고 배경)
  - `맞춤법` / `기록` / `자동변환` | `A-` `A+` (글자 크기, 4단계) | `AI` | `집중` (집중 모드 진입)
- 목표 달성 시 (percent ≥ 100): 진행바 색상 인디고 → 에메랄드 변경 + "🎉 목표 달성!" 메시지
...
| `NovelCard` | `src/components/NovelCard.tsx` | 홈화면 작품 카드 — 컬러 상단 바, 제목(인라인 수정), 설명, 회차 수 뱃지(컬러), 상대 시간, 호버 시 컬러 팔레트(6색) |
| `ShortcutsModal` | `src/components/ShortcutsModal.tsx` | 에디터 단축키 도움말 모달 (backdrop 클릭 또는 ✕로 닫기) |
| `ErrorBoundary` | `src/components/ErrorBoundary.tsx` | JS 오류 캐치 → 안내 화면 표시 (다시 시도 / 홈으로 이동) |
